...
const tableDivs = document.getElementById("tables");

// getReadyTables().map(tableId => {
//     tableDivs.innerHTML += `<div class="table">${tableId}</div>`}).join(',');

let readyTs = getReadyTables();
let tableIds = readyTs.map(tableNum => {
    tableDivs.innerHTML += `<div class = "table">${tableNum}</div>`}
    );
console.log(readyTs);
}
